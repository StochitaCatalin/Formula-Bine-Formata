...
using static FormulaBineFormata.MainWindow;
...
namespace FormulaBineFormata
{
    public partial class MainForm : Form
    {
        ...
        private List<SampleDataModel> NodToData(Nod nd,string parent)
        {
            ...
            int pos = i;
            if (nd.ch.Count == 3)
                sampleTree.Add(new SampleDataModel { Id = pos.ToString(), ParentId = parent, Name = nd.ch[1].ToString() });
            else
                sampleTree.Add(new SampleDataModel { Id = pos.ToString(), ParentId = parent, Name = nd.ch[0].ToString() });
            i++;
            if (nd.nods.Count > 0)
